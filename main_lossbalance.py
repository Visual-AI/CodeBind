...
from models.loss_balance import LossBalancer
...
class CodeBind(L.LightningModule):
    ...
    def set_loss_weight(self):
        ...
        loss_names = ['info_nce'] # 基准
        if self.use_vq:
            loss_names.extend(['loss_vq', 'loss_vq_reg','loss_cmcm'])

        loss_names.extend(['loss_modal_decomp','loss_uniform'])

        if self.use_decoder:
            loss_names.extend(['loss_rec'])

        # 2. 初始化 Balancer
        self.balancer = LossBalancer(
            loss_names=loss_names,
            base_loss_name='info_nce',
            momentum=0.9,
            min_interval=10,   # 初始间隔
            max_interval=100,  # 最大间隔
            max_steps=40000   # 间隔增长率
        )
        logging.info(f"Initialized LossBalancer with keys: {loss_names}")
    ...
    def calculate_loss(self, out_dict, batch, batch_idx, mode):
        ...
        if self.use_vq:
            ...
            raw_losses_dict.update(loss_vq)
        ...
        loss_nll = self.info_nce_loss(feats_anchor, feats_b, mode=mode)
        raw_losses_dict['info_nce'] = loss_nll

        if self.use_decoder:
            loss_recons = self.rec_loss(out_dict, batch, batch_idx, mode=mode)
            raw_losses_dict['loss_rec'] = loss_recons

        is_training = (mode == 'train')
        total_loss, weight_logs = self.balancer.get_weighted_loss(raw_losses_dict, is_training=is_training)

        if is_training:
            self.log_dict(weight_logs, prog_bar=False, on_step=True, on_epoch=False)
    ...
    def vq_loss(self, vq_dict, mode="train"):
        ...
        # 1. Basic VQ Losses (Codebook + Commitment)
        ...
        raw_losses['loss_vq'] = loss_vq
        self.log(mode + "_loss_vq", loss_vq, prog_bar=True, on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, sync_dist=True)

        if self.args.get('loss_vq_reg_weight'):
            raw_losses['loss_vq_reg'] = loss_vq_contra + loss_vq_uni
        ...
        if self.args.get('loss_cmcm_weight'):
            loss_cmcm = self.cmcm_loss(vq_dict)
            raw_losses['loss_cmcm'] = loss_cmcm
            self.log(mode + "_loss_cmcm", loss_cmcm, prog_bar=True, on_step=LOG_ON_STEP, on_epoch=LOG_ON_EPOCH, sync_dist=True)
        
        if self.args.get('loss_modal_decomp_weight') and self.cfg_vq.get('codebook_num') > 1:
            loss_modal_decomp = self.modal_decomp_loss(vq_dict)
            raw_losses['loss_modal_decomp'] = loss_modal_decomp
        ...
        if self.args.get('loss_uniform_weight') and self.cfg_vq.get('codebook_num') > 1:
            loss_uniform = self.uniform_loss(vq_dict)
            raw_losses['loss_uniform'] = loss_uniform
...
def set_logger(args):
    ...
    if logger == "wandb":
        ...
        raise NotImplementedError(f"logger {logger} is not implemented yet. Please choose from ['wandb']")
