# timestamp=$(date "+%Y%m%d_%H%M%S")
# echo ${timestamp}


# expname=p365_lora_MVQ1024x8_256x8
# expdir=exp_p365


# mkdir -p exp/${expdir}/${expname}


# # --- --- --- --- --- --- --- ---   Train  --- --- --- --- --- --- --- ---  
# python main.py \
# --expname ${expname} \
# --scale_factor 1 \
# --batch_size 10 \
# --max_epochs 40 \
# --loggers wandb \
# --val_loss True \
# --cfg config/cfg_p365.yaml \
# --cfg_model config/cfg_model.yaml \
# --checkpoint_dir exp/${expdir}/${expname}/checkpoints \
# --log_img_dir exp/${expdir}/${expname}/log_img \
# --lightning_log exp/${expdir}/${expname}/lightning_log \
# --save_cfg_filepath exp/${expdir}/${expname}/cfg_all_train.yaml \
# --loggers_dir exp/${expdir}/${expname}/log \
# --device cuda:0 cuda:1 cuda:2 2>&1 | tee ./exp/${expdir}/${expname}/${timestamp}_train.log


# --- --- --- --- --- --- --- ---  Test   --- --- --- --- --- --- --- ---  
# timestamp=$(date "+%Y%m%d_%H%M%S")
# echo ${timestamp}
# expname=exp/exp_p365/p365_eval_by_p365_lora
# mkdir -p ${expname}
# python main.py \
# --expname ${expname} \
# --resume_expname /home/vislab/jieli23/works/vitlens/exp_from_zilong/exp_p365/p365_lora_MVQ1024x8_256x8 \
# --train False \
# --batch_size 20 \
# --cfg config/cfg_p365.yaml \
# --device cuda:0 2>&1 | tee ${expname}/${timestamp}_test.log


# # --- --- --- --- --- --- --- ---  Test   --- --- --- --- --- --- --- ---  
timestamp=$(date "+%Y%m%d_%H%M%S")
echo ${timestamp}
expname=exp/exp_p365/p365_eval_by_p365_lora
mkdir -p ${expname}
python main.py \
--expname ${expname} \
--resume_expname /home/vislab/jieli23/works/vitlens/exp_from_zilong/exp_p365/p365_lora_MVQ1024x8_256x8_exp_v1 \
--train False \
--batch_size 20 \
--cfg config/cfg_p365.yaml \
--device cuda:0 2>&1 | tee ${expname}/${timestamp}_test_exp_v1.log

# TODO p365 headtune