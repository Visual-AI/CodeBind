...
class SelectElement(nn.Module):
    ...
    def forward(self, x):
        assert x.ndim >= 3
        # pdb.set_trace()
        if self.index == -1: # bypass SelectElement to return the whole tensor
            return x
        else:
            return x[:, self.index, ...]
...
class SelectEOSAndProject(nn.Module):
    ...
    def __init__(self, proj: nn.Module, only_proj=False) -> None:
        super().__init__()
        self.proj = proj
        self.only_proj = only_proj

    def forward(self, x, seq_len):
        assert x.ndim == 3
        # x is of shape B x L x D
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        if not self.only_proj:
